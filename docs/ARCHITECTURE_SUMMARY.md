...
- Enforces **Identity Lock** via Redundant Identity Stack (5 layers of defense):
  1. LoRA (model bias) + 2. IP-Adapter (per-frame anchor) + 3. ControlNet (structure)
  4. Face Enhancement (post-processing backup) + 5. ArcFace (QA verification)
  See ARCHITECTURE.md Section 3A.3 for full details.
...
|----------|--------|---------|----------|
| **I2V-First (Recommended)** | Keyframe --> I2V | Bridge --> I2V | Professional filmmaking, maximum consistency |
| **T2V + I2V (Alternative)** | T2V | Bridge --> I2V | Exploration mode, "surprise me" users |

**Why I2V-First is Preferred:**
1. **Consistent quality**  No jarring T2V-->I2V quality shift (especially with different param models)
2. **Better composition control**  User/system defines shot 1's framing via keyframe
3. **Stronger identity from frame 1**  Keyframe can include character with IP-Adapter
4. **Bridge Engine pattern extended**  Shot 1 keyframe uses same workflow as bridge frames

**Keyframe Generation for Shot 1:**
```
User provides keyframe --> Use directly as init_image
No keyframe provided --> Generate via SDXL + IP-Adapter (same as Bridge Engine)
Exploration mode --> T2V generates options, user picks, becomes keyframe
```
...
```python
# User/config chooses renderer  orchestration code unchanged
...
| **Pro** | SDXL keyframe --> Runway/Veo | Runway/Veo + Bridge | Premium output |

*Note: T2V path preserved for exploration/legacy but not promoted as default.*

---

### [!] T2V vs I2V: Development vs MVP Transition
...
#### BRIDGE FRAME: CRITICAL COMPONENT DO NOT BYPASS
...
| Chunk 1 --> Chunk 2 (same shot, 12s max) | [OK] YES | Generation restart |
| Repair/patch a bad frame | [OK] YES | New generation |
| Focus change (Person A --> Person B) | [OK] YES | Different subject |
| Re-roll after audit failure | [OK] YES | New generation |
| Continue from checkpoint | [OK] YES | New generation |
| Within a single 12s chunk | [X] NO | Continuous generation |
...
|----------|----------|--------|
| `bridge_full.json` | ControlNet Pose + Depth + IP-Adapter | [OK] RECOMMENDED |
| `bridge_pose_only.json` | ControlNet Pose + IP-Adapter | [*] Fallback |
| `bridge_ipadapter.json` | IP-Adapter only | [*] Minimal fallback |
| `bridge_basic.json` | SDXL img2img only | [X] BROKEN - Do not use |
...
| 4 (Emergency) | Raw frame --> Wan I2V | `pass1_img2vid.json` | [!] DRIFT WILL OCCUR |
...
| **IP-Adapter in I2V** | Only in hero/bridge frames | `pass1_img2vid_ipadapter.json` with IP-Adapter-WAN | Within-shot drift (~3%) not prevented |
| **Face Enhancement** | Not implemented | GFPGAN/CodeFormer post-processing | No safety net for residual drift |
| **Stand-In for Wan** | Not tested | Evaluate vs IP-Adapter | May offer better native identity |
...
| **Redundant Identity Stack** | 5-layer defense system for identity: LoRA (bias) + IP-Adapter (anchor) + ControlNet (structure) + Face Enhancement (backup) + ArcFace (QA). See ARCHITECTURE.md 3A.3. |
| **Stand-In** | Tencent research project adding native identity branch to Wan models (~1% params). Alternative to IP-Adapter with potentially better integration. |
